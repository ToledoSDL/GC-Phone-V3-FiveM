...
func = Tunnel.getInterface("gcphone")

src = {}
vRP = Proxy.getInterface("vRP")
vRPserver = Tunnel.getInterface("vRP")
Tunnel.bindInterface("gcphone", src)
Proxy.addInterface("gcphone", src)
		...
		if IsControlJustPressed(1,311) then
			hasPhone(function(hasPhone)
				if hasPhone == true then
					...
					TriggerServerEvent("vRP/update_gc_phone")
					TooglePhone()
				end
			end)
		end
...
RegisterNetEvent("gcPhone:forceClosePhone")
AddEventHandler("gcPhone:forceClosePhone",function()
	if menuIsOpen == true then
		TooglePhone()
	end
end)
